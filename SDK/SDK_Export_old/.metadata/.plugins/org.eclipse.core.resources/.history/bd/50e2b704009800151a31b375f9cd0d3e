...
int main()
{
	...
	for(i = 0; i < size*NUM_BLOCK; i++){
		if (i % 4 == 0){
			result[k++] = result_addr[i/4] & 0xff;
		}
		else if (i % 4 == 1){
			result[k++] = (result_addr[i/4] >> 8) & 0xff;
		}
		else if (i % 4 == 2){
			result[k++] = (result_addr[i/4] >> 16) & 0xff;
		}
		else{
			result[k++] = (result_addr[i/4] >> 24) & 0xff;
		}
	}
	...
	if (m == 0) xil_printf("FAILED\n");
	else xil_printf("PASSED\n");
